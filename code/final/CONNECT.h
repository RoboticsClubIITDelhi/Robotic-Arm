...
static bool danceExecuted = false;
static unsigned long lastDanceTime = 0;
...
void activeCtrl(int cmdInput){
  switch(cmdInput){
    ...
    case 22: 
        // Prevent continuous execution with debouncing
        if(!danceExecuted && (millis() - lastDanceTime > 2000)) {
            danceExecuted = true;
            dance(); 
            danceExecuted = false;
            lastDanceTime = millis();
            // Reset flag after dance duration
            // You can also reset this in the dance() function itself
        }
      break;
    case 24:
      st.WritePosEx(3, st.ReadPos(3)+20, 1000,0);
      break;
    case 25:
      st.WritePosEx(3, st.ReadPos(3)-20, 1000,0);
      break;
    case 26:
      st.WritePosEx(1, st.ReadPos(1)-25, 500,0);
      break;
    case 27:
      st.WritePosEx(1, st.ReadPos(1)+25, 500,0);
      break;
...
void webCtrlServer(){
    ...
    switch(cmdT){
      ...
      case 23: 
        Serial.println("Executing IK with parameters:");
        Serial.print("R: "); Serial.println(cmdI);
        Serial.print("Theta: "); Serial.println(cmdA);
        Serial.print("Z: "); Serial.println(cmdB);
        ik(cmdI, cmdA, cmdB);
          break;
        default:
